...
travtables_t	travtables = {
	...
	/* TR_prt */
	,{&TRAVerror, &PRTprogram, &PRTdeclarations, &PRTfundec, &PRTfundefs, &PRTfundef, &PRTfunheader, &PRTglobaldec, &PRTglobaldef, &PRTparam, &PRTfunbody, &PRTvardec, &PRTifelse, &PRTwhile, &PRTdowhile, &PRTfor, &PRTreturn, &PRTexpressions, &PRTids, &PRTstmts, &PRTassign, &PRTbinop, &PRTmonop, &PRTvarlet, &PRTvar, &PRTnum, &PRTfloat, &PRTbool, &PRTsymboltableentry, &PRTerror}

	/* TR_copy */
	,{&TRAVerror, &COPYprogram, &COPYdeclarations, &COPYfundec, &COPYfundefs, &COPYfundef, &COPYfunheader, &COPYglobaldec, &COPYglobaldef, &COPYparam, &COPYfunbody, &COPYvardec, &COPYifelse, &COPYwhile, &COPYdowhile, &COPYfor, &COPYreturn, &COPYexpressions, &COPYids, &COPYstmts, &COPYassign, &COPYbinop, &COPYmonop, &COPYvarlet, &COPYvar, &COPYnum, &COPYfloat, &COPYbool, &COPYsymboltableentry, &COPYerror}

	/* TR_free */
	,{&TRAVerror, &FREEprogram, &FREEdeclarations, &FREEfundec, &FREEfundefs, &FREEfundef, &FREEfunheader, &FREEglobaldec, &FREEglobaldef, &FREEparam, &FREEfunbody, &FREEvardec, &FREEifelse, &FREEwhile, &FREEdowhile, &FREEfor, &FREEreturn, &FREEexpressions, &FREEids, &FREEstmts, &FREEassign, &FREEbinop, &FREEmonop, &FREEvarlet, &FREEvar, &FREEnum, &FREEfloat, &FREEbool, &FREEsymboltableentry, &FREEerror}

	/* TR_chk */
	,{&TRAVerror, &CHKprogram, &CHKdeclarations, &CHKfundec, &CHKfundefs, &CHKfundef, &CHKfunheader, &CHKglobaldec, &CHKglobaldef, &CHKparam, &CHKfunbody, &CHKvardec, &CHKifelse, &CHKwhile, &CHKdowhile, &CHKfor, &CHKreturn, &CHKexpressions, &CHKids, &CHKstmts, &CHKassign, &CHKbinop, &CHKmonop, &CHKvarlet, &CHKvar, &CHKnum, &CHKfloat, &CHKbool, &CHKsymboltableentry, &CHKerror}
